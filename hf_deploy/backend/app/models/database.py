...
def get_connection():
    ...
    # --- Fix L-1: WAL mode on every connection ---
...
def create_job(job_id: str, file_path: str, original_filename: str,
    ...
    # --- Fix L-3: Timezone-aware timestamps ---
...
def get_job_safe(job_id: str) -> Optional[Dict[str, Any]]:
    """Returns job data without exposing internal file_path (Fix H-6)."""
    job = get_job(job_id)
    if job:
...
def try_set_job_processing(job_id: str) -> bool:
    """Fix L-2: Atomically set status to 'processing' only if not already processing.
    Returns True if status was set, False if already being processed."""
    now = datetime.now(timezone.utc).isoformat()
